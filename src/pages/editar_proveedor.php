<?php
    ...
    if($proveedor_id) {
        ...
        if(!$proveedor) {
            echo json_encode(["error" => "Proveedor no encontrado.", "icon" => "error"]);
            exit();
        }
    }

    $estatus = (int)($proveedor['estatus'] ?? 0);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        try {
            ...
            if (!filter_var($correo, FILTER_VALIDATE_EMAIL)) {
                ...
                try {
                    // excluir el proveedor actual al verificar unicidad de correo
                    $stmt = $pdo->prepare("SELECT COUNT(*) FROM proveedores WHERE correo = :correo AND id_proveedor != :id");
                    $stmt->execute(['correo' => $correo, 'id' => $proveedor_id]);
            ...
            // Iniciar transacción como en editar_empleado
            if (!$pdo->inTransaction()) $pdo->beginTransaction();
            $sql = "UPDATE proveedores 
                    SET id_proveedor = :id_proveedor, nombre = :nombre, apellido_paterno = :apellido_paterno, apellido_materno = :apellido_materno, empresa = :empresa,
                    celular = :celular, correo = :correo, estatus = :estatus
            ...
            $pdo->commit();
            echo json_encode(["success" => "Proveedor actualizado correctamente.", "redirect" => "index.php?view=proveedores", "icon" => "success"]);
            exit();
        } catch (Exception $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            echo json_encode(["error" => "Error al actualizar el proveedor: " . $e->getMessage(), "icon" => "error"]);
            exit();
        }
    }
?>
...
<body>
    <div class="form-container animate-in">
        <div class="form-card">
            ...
            <!-- Form -->
            <form id="editar" action="index.php?view=editar_proveedor" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="id" value="<?= htmlspecialchars($proveedor_id) ?>">
                <div class="form-body">
    ...
    <script>
        $(document).ready(function() {
            $('#editar').on('submit', function(e) {
                ...
                $.ajax({
                    url: "index.php?view=editar_proveedor",
                    type: "POST",
                    data: $(this).serialize(),
