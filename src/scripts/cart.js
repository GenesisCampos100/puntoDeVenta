...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateCart() {
    ...
    if (selectedClient) {
      ...
      if (clienteNombreEl) clienteNombreEl.textContent = translations.no_customer_selected || 'No hay cliente seleccionado';
      if (clienteIdInput) clienteIdInput.value = '';
      if (hiddenIdClienteForm) hiddenIdClienteForm.value = '';
    }

    if (!cart.length) {
      cartContainer.innerHTML = `<div class="text-center text-gray-500 py-10"><p class="text-lg font-medium">${translations.empty_cart_message}</p><p class="text-sm mt-2">${translations.add_products_message}</p></div>`;
      recalcTotals();
      return;
    }
                ...
                <div class="flex flex-col w-full">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex gap-2">
                            <button class="discount-btn text-blue-600 hover:underline text-sm">${translations.discount_button}</button>
                            <button class="remove-btn text-red-600 hover:underline text-sm">${translations.remove_button}</button>
                        </div>
                    </div>
